    ...
    export class HTMLRendererService {
        ...
        public configLoaded() {
            ...
            Object.keys(this.configService.js_wrap_map)
                .forEach((key) => {
                    template = this.configService.js_wrap_map[key];
                    this.iframeTemplateRenderers[key] = this.$interpolate(template);
                });
            ...
            if (resourceResult) {
                ...
                htmlContext = {
                    ...
                    styles: frameworkFiles.css.map((cssFile: string) => {
                        return '<link rel="stylesheet" type="text/css" href="data:text/html;base64,' +
                            resourceResult[project.options.inlineProxyURL + cssFile] +
                            '"/>\n';
                    }).join(''),
                    js: js,
                    scripts: frameworkFiles.js.map((jsFile: string) => {
                        return '<script src="data:text/html;base64,' +
                            resourceResult[project.options.inlineProxyURL + jsFile] + '"></script>\n';
                    }).join('')
                };
